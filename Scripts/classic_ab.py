...
class classics:
    ...
    def get_group_result(self,df: pd.DataFrame, from_column: str, val_type: str, value_column: str) -> pd.Series:
    # Retrieve the group results
        group_results = df[df[from_column] == val_type][value_column]
        return group_results


    def get_count(self,group_result: pd.Series) -> int:
        return group_result.count()


    def get_sum(self,group_result: pd.Series) -> int:
        return group_result.sum()


    def form_success(self,control_group_result, treatment_group_result) -> list:
        control_sum = self.get_sum(control_group_result)
        treatment_sum = self.get_sum(treatment_group_result)
        return [control_sum, treatment_sum]


    def form_noob(self,control_group_result, treatment_group_result) -> list:
        control_count = self.get_count(control_group_result)
        treatment_count = self.get_count(treatment_group_result)
    ...
    def get_lower_upper_bounds(self,successes: list, nobs: list, alpha: float = 0.05) -> list:
    ...
    def print_results(self, ztest_result: tuple, lower_upper_bound_result: list) -> None:
